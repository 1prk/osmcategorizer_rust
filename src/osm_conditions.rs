...
pub struct Conditions<'a> {
    pub tags: &'a mut HashMap<&'a str, &'a str>,
}

// hier beginnt die Implementierung der Methoden für die struct Conditions
impl<'a> Conditions<'a> {
    pub(crate) fn new(tags: &'a mut HashMap<&'a str, &'a str>) -> Conditions<'a> {
        Conditions { tags }
    }
    pub fn set_infra<'b>(&mut self, infrastructure: &'a str) -> &'a str {
        self.tags.insert("bicycle_infrastructure", infrastructure);
        infrastructure
    }
    ...
    pub fn can_walk(&self, direction: &str) -> bool {
        self.tags.get("foot").map(|v| v == &"yes" || v == &"designated").unwrap_or(false)
            ...
            || self.tags.get(&(format!("sidewalk:{}", direction).as_str())).map(|v| v == &"yes" || v == &"separated" || v == &"both" || v == &"left").unwrap_or(false)
    ...
    pub fn is_designated_bicycle(&self, direction: &str, mode: &str) -> bool {
        let sidewalk: &str = &format!("sidewalk:{}", mode);
        let cycleway: &str = &format!("cycleway:{}", mode);
        let sidewalk_directional: &str = &format!("sidewalk:{}:{}", direction, mode);
        let cycleway_directional: &str = &format!("cycleway:{}:{}", direction, mode);
    ...
    pub fn is_smooth(&self) -> bool {
        match self.tags.get("tracktype") {
            Some(tracktype) => vec!["grade1", "grade2"].contains(&tracktype),
    ...
    pub fn is_path_not_forbidden(&self) -> bool {
        match self.tags.get("highway") {
            Some(highway) => {
                let not_forbidden = ["cycleway", "track", "path"].contains(&highway);
    ...
    pub fn is_bikepath(&self, direction: &str) -> bool {
        let bicycle_directional: &str = &format!("{}:bicycle", direction);
        let cycleway_directional: &str = &format!("cycleway:{}", direction);
        let lane_directional: &str = &format!("cycleway:{}:lane", direction);
        let trafficsign_directional: &str = &format!("{}:traffic_sign", direction);
        ...
        && !self.tags.iter().any(|(k, _)|  k.contains(&lane_directional)))
    ...
    pub fn is_bikelane(&self, direction: &str) -> bool {
        let cycleway_direction: &str = &format!("cycleway:{}", direction);
        let lane_direction: &str = &format!("{}:lane", direction);
